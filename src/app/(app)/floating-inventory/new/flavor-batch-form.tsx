...
import { createFloatingFlavorBatchAction, type ActionState } from "../actions";
...
export function NewFloatingFlavorBatchForm({ brands }: { brands: string[] }) {
  const [state, formAction, pending] = useActionState(createFloatingFlavorBatchAction, initialState);
  ...
  return (
    <form action={formAction} className="flex flex-col gap-6">
      ...
      <label className="flex flex-col gap-1.5">
        <Label>Quantity (per flavor)</Label>
        <Input name="quantity" type="number" min={1} placeholder="e.g. 10" required />
        <p className="mt-1 text-xs text-muted">
          Applied to every flavor created below — add more later using &quot;Add stock&quot;.
        </p>
      </label>
