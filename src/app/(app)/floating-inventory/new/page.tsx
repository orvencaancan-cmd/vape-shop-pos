...
export default async function NewFloatingProductPage({
  ...
  if (mode === "new-category") {
    return (
      <PageShell
        ...
        backHref="/floating-inventory/new"
  ...
  const { data: brandRows } = await supabase.from("floating_products").select("brand");
  ...
  if (categoryKey && categoryKey !== "ejuice") {
    ...
    if (categoryKey === "flavor-pod") {
      return (
        ...
        >
          <NewFloatingFlavorPodBatchForm brands={brands} />
        </PageShell>
      );
    }
  ...
  if (categoryKey === "ejuice") {
    return (
      <PageShell
        title="E-juice"
        subtitle="Pick a brand, then fill in quantity, cost, price, and flavors separately for each nicotine level you carry."
        backHref="/floating-inventory/new"
        backLabel="Change category"
      >
        <NewFloatingFlavorBatchForm brands={brands} />
      </PageShell>
    );
  }
  ...
  return (
    <PageShell title="What are you adding?">
      <div className="grid grid-cols-2 gap-3">
        <Link href="/floating-inventory/new?category=ejuice">
        ...
        <Link href="/floating-inventory/new?mode=new-category">
          <Card
            padding="lg"
