...
export async function getCurrentProfile(): Promise<CurrentProfile | null> {
  ...
  const { data: memberships } = await supabase
    .from("profiles")
    .select(
      "id, shop_id, role, platform_admin, display_name, shops(id, name, subscription_status, trial_ends_at, current_period_end, billing_tier, logo_url, primary_color, suspended_at, archived_at, is_platform_shop)",
    )
    .eq("user_id", user.id)
    .order("created_at");
  ...
  return {
    ...
    shop: {
      ...
      currentPeriodEnd: activeShop.current_period_end,
